...
def _get_backend() -> str:
    ...
    If no API backend is configured but Scrapling is available, falls back
    to local Scrapling direct extraction (no API key required).
    """
    ...
    backend_candidates = (
        ...
        ("scrapling", _is_scrapling_available()),
    )
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "scrapling":
        return _is_scrapling_available()
    if backend == "searxng":
...
def _is_scrapling_available() -> bool:
    """Check if Scrapling library is installed and available."""
    try:
...
def _web_requires_env() -> list[str]:
    ...
    Scrapling requires no API keys since it runs locally.
    """
    configured_backend = _get_backend()
    if configured_backend == "scrapling":
        # No env vars needed for local Scrapling extraction
        return []
    return [
        "EXA_API_KEY",
        "PARALLEL_API_KEY",
...
async def _scrapling_extract(urls: List[str]) -> List[Dict[str, Any]]:
    """Extract content from URLs using local Scrapling library (no API key needed).

    Scrapling provides built-in stealth against bot detection and automatic retries,
    making it ideal for scraping sites that block normal requests.

    This uses the full StealthyFetcher backend (Playwright Chromium-based) which runs
    a real browser for maximum stealth against advanced anti-bot detection.
    """
    from scrapling import StealthyFetcher
    from tools.interrupt import is_interrupted

    logger.info("Scrapling extract (local StealthyFetcher browser): %d URL(s)", len(urls))
    results: List[Dict[str, Any]] = []

    # Use StealthyFetcher with default stealth settings
    ...
    for url in urls:
        ...
        if blocked:
            logger.info("Blocked Scrapling extract for %s by rule %s", blocked["host"], blocked["rule"])
            results.append({
        ...
        try:
            # Scrapling automatically handles retries and stealth
            logger.info("Scraping with Scrapling (StealthyFetcher browser): %s", url)
            # Run synchronous fetch in thread with 60s timeout
            response = await asyncio.wait_for(
                asyncio.to_thread(fetcher.fetch, url, timeout=30000),
                timeout=60,
            )

            # Extract content from response
            # Scrapling has built-in stealth that bypasses most anti-bot protection
            full_html = response.html_content
            # Get title from HTML
            title = response.css("title").extract_first().strip() if response.css("title") else ""
            # Extract title tag content (strip the <title> tags)
            if title.startswith("<title>") and title.endswith("</title>"):
                title = title[7:-8].strip()

            # Scrapling automatically converts relative URLs to absolute
            # We just extract the full HTML content (similar to Firecrawl html format)
            results.append({
                "url": response.url,
                "title": title,
                "content": full_html,
                "raw_content": full_html,
                "metadata": {"sourceURL": response.url, "title": title},
            })
            logger.info("Scrapling extract succeeded: %s (%d chars)", url, len(full_html))

        except asyncio.TimeoutError:
            logger.warning("Scrapling extract timed out for %s", url)
            results.append({
                "url": url, "title": "", "content": "",
                "error": "Scrape timed out after 60s — page may be too large or unresponsive.",
            })
        except Exception as e:
            logger.debug("Scrapling extract failed for %s: %s", url, str(e))
            results.append({
                "url": url,
                "title": "",
                "content": "",
                "raw_content": "",
                "error": f"Scrapling extract failed: {str(e)}"
            })
...
async def process_content_with_llm(
    ...
    For very large content (greater than 500k chars), uses chunked processing with synthesis.
    For extremely large content (greater than 2M chars), refuses to process entirely.
    ...
    try:
        ...
        if not safe_urls:
            ...
            if backend == "parallel":
                ...
                results = await _scrapling_extract(safe_urls)
            else:
...
if __name__ == "__main__":
    ...
    scrapling_available = _is_scrapling_available()
    ...
    if web_available:
        ...
        if backend == "exa":
            ...
            print("   Using local Scrapling (no API key required)")
        else:
        ...
        if scrapling_available:
            print("✅ No API keys configured, but Scrapling is installed")
            print("   Falling back to local extraction with Scrapling (works without API keys)")
        else:
    ...
    if not web_available and not scrapling_available:
        exit(1)
    ...
    if scrapling_available:
        print("🕵️  Scrapling stealth scraping available (local, no API key needed)")
